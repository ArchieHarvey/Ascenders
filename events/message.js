...
client.on('message', async message => {
  ...
  try {
    ...
    let prefix = message.content.match(prefixMention) ? message.content.match(prefixMention)[0] : !guildDB.prefix ? config.prefix : guildDB.prefix;
    ...
    if (!message.channel.nsfw && cmd.nsfw) {
      return message.channel.send("Oh no. Why are you running a NSFW command in a SFW channel.");
    }

    //If command is owner only and author isn't owner return
    if (cmd.ownerOnly && message.author.id !== config.ownerID) {
      return message.channel.send("Looks like you have found a hidden command. Sorry its reserved for the bot owners");
    }
