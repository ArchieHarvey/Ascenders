...
module.exports = {
    name: "color",
    description: "Shows information about a hex color",
    usage: "",
    enabled: true,
    aliases: ["v", "vol"],
    category: "Music",
    ...
    async execute(client, message, args, data) {
        ...
        if (message.mentions.members.first()) {
            ...
            if (/(#|0x)([0-9A-F]{6})/i.test(args[0])) {
                ...
                const hexerr = new MessageEmbed()
                .setAuthor('Error')
                .setDescription(`Make sure its hex | Example: #FF0000 | 0xFF0000\n\nInvalid command usage, try using it like: \n \`${data.guild.prefix}color [hex]\` \n\n Arguments: \n \`hex\`: *hex of the color*`)
                message.channel.send(hexerr)
                return
            }
        } else {
